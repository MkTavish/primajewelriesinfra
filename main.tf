...
EOD
  ...
  provisioner "local-exec" {
    command = "aws ec1 wait instance-status-ok --instance-ids ${aws_instance.wp_dev.id} --profile maktavish && ansible-playbook -i aws_hosts wordpress.yml"
  }
}
...
$!/bin/bash
cat <<EOF > userdata
/usr/bin/aws s3 sync s3://${aws_s3_bucket.code.bucket} /var/www/html/bin/touch /var/spool/cron/root
